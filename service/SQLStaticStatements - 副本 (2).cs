...
SUM( ISNULL(A1.FQTY1,0) - ISNULL(A3.FQTY3,0)+ISNULL(A5.FQTY5,0) - ISNULL(A7.FQTY7, 0) - ISNULL(A9.FQTY9, 0) - ISNULL(A11.FQTY11, 0) + ISNULL(A13.FQTY13,0) + ISNULL(A19.FQTY19,0) + ISNULL(A15.FQTY15,0) - ISNULL(A17.FQTY17,0) + ISNULL(A21.FQTY21,0) + ISNULL(A23.FQTY23,0) - ISNULL(A25.FQTY25,0)) as TOTPROD 
...
WHERE T2.FDATE <= '{1}'  AND T2.FDOCUMENTSTATUS ='C' AND T1.FSTOCKID=281715
...
WHERE T4.FDATE <= '{1}' AND T4.FDOCUMENTSTATUS ='C' AND T3.FSRCSTOCKID = 281715
...
WHERE T6.FDATE <= '{1}'  AND T6.FDOCUMENTSTATUS ='C' AND T5.FSTOCKID=281715
GROUP BY T5.FMATERIALID ) A5 ON A5.FMIDA5 = T.FMATERIALID

LEFT JOIN 
(
  SELECT T7.FMATERIALID AS FMIDA7,
   SUM(ISNULL(T71.FBASEJOINTRANQTY, 0)) AS FQTY7 
FROM T_STK_STKTRANSFEROUTENTRY T7 
LEFT JOIN T_STK_STKTRANSFEROUT T8 ON T7.FID=T8.FID
INNER JOIN T_STK_STKTRANSFEROUTENTRY_T t71 on t71.FENTRYID = t7.FENTRYID
WHERE T8.FDATE <= '{1}'  AND T8.FDOCUMENTSTATUS ='C' AND T7.FSRCSTOCKID=281715
...
WHERE T10.FDATE <= '{1}'  AND T10.FDOCUMENTSTATUS ='C' AND T9.FSTOCKID=281715
...
WHERE T12.FDATE <= '{1}'  AND T12.FDOCUMENTSTATUS ='C' AND T11.FSTOCKID=281715
...
(
SELECT
  T13.FMATERIALID AS FMIDA13,
  SUM(ISNULL(T131.FBaseTransferQty,0)) AS FQTY13
FROM 
  T_STK_STKTRANSFERINENTRY T13 
LEFT JOIN T_STK_STKTRANSFERIN T14 ON T13.FID = T14.FID 
LEFT JOIN T_STK_STKTRANSFERINENTRY_T T131 ON T131.FENTRYID =  T13.FENTRYID 
WHERE T14.FDATE <='{1}' AND T14.FDOCUMENTSTATUS ='C' AND T13.FSRCSTOCKID = 281715
GROUP BY T13.FMATERIALID ) A13 ON A13.FMIDA13 = T.FMATERIALID
...
WHERE TA2.FDATE <='{1}' AND TA2.FDOCUMENTSTATUS ='C' AND TA1.FSTOCKID=281715
...
WHERE TA4.FDATE <='{1}' AND TA4.FDOCUMENTSTATUS ='C' AND TA3.FSTOCKID=281715
...
WHERE TA6.FDATE <='{1}'  AND TA6.FDOCUMENTSTATUS ='C' AND TA5.FSTOCKID=281715
...
WHERE TA10.FDATE <= '{1}' AND TA10.FDOCUMENTSTATUS ='C' AND TA9.FSTOCKID=281715
...
WHERE TA12.FDATE <= '{1}' AND TA12.FDOCUMENTSTATUS ='C' AND TA11.FSTOCKID=281715
...
SELECT FMATERIALID AS FMID,
SUM( ISNULL(A1.FQTY1,0) - ISNULL(A3.FQTY3,0)+ISNULL(A5.FQTY5,0) - ISNULL(A7.FQTY7, 0) - ISNULL(A9.FQTY9, 0) - ISNULL(A11.FQTY11, 0) + ISNULL(A13.FQTY13,0) + ISNULL(A19.FQTY19,0) + ISNULL(A15.FQTY15,0) - ISNULL(A17.FQTY17,0) + ISNULL(A21.FQTY21,0) + ISNULL(A23.FQTY23,0) - ISNULL(A25.FQTY25,0)) as TOTPROD 
--SUM( ISNULL(A1.FQTY1,0) +ISNULL(A5.FQTY5,0) - ISNULL(A9.FQTY9, 0) - ISNULL(A11.FQTY11, 0)  + ISNULL(A19.FQTY19,0) + ISNULL(A15.FQTY15,0) - ISNULL(A17.FQTY17,0) + ISNULL(A21.FQTY21,0) + ISNULL(A23.FQTY23,0) - ISNULL(A25.FQTY25,0)) as TOTPROD 
FROM T_BD_MATERIAL T
LEFT JOIN
(SELECT
  T1.FMATERIALID AS FMIDA1,
...
(SELECT
  T13.FMATERIALID AS FMIDA13,
  SUM(ISNULL(T13.FQTY,0)) AS FQTY13
FROM 
  T_STK_STKTRANSFERINENTRY T13 
LEFT JOIN T_STK_STKTRANSFERIN T14 ON T13.FID = T14.FID 
WHERE T14.FDATE < '{1}' AND T14.FDOCUMENTSTATUS ='C' AND T13.FDESTSTOCKID = 281715
GROUP BY T13.FMATERIALID ) A13 ON A13.FMIDA13 = T.FMATERIALID
...
(
SELECT FMATERIALID AS FMID,
SUM( ISNULL(A1.FQTY1,0) + ISNULL(A3.FQTY3,0)+ISNULL(A7.FQTY7,0)) as TOTPROD 
FROM T_BD_MATERIAL T
LEFT JOIN
(SELECT
  T1.FMATERIALID AS FMIDA1,
...
T_STK_MISCELLANEOUSENTRY T1 
LEFT JOIN T_STK_MISCELLANEOUS T2 ON T1.FID = T2.FID 
WHERE T2.FDATE BETWEEN '{1}' AND '{2}' AND T2.FDOCUMENTSTATUS ='C' AND T1.FSTOCKID=281715
GROUP BY T1.FMATERIALID )  A1 ON A1.FMIDA1 = T.FMATERIALID
...
FROM 
  T_STK_STKTRANSFERINENTRY T3 
LEFT JOIN T_STK_STKTRANSFERIN T4 ON T3.FID = T4.FID 
WHERE T4.FDATE BETWEEN '{1}' AND '{2}' AND T4.FDOCUMENTSTATUS ='C' AND T3.FDESTSTOCKID = 281715
GROUP BY T3.FMATERIALID ) A3 ON A3.FMIDA3 = T.FMATERIALID
...
WHERE T8.FDATE BETWEEN '{1}' AND '{2}' AND T8.FDOCUMENTSTATUS ='C' AND T7.FSTOCKID=281715
GROUP BY T7.FMATERIALID ) A7 ON A7.FMIDA7 = T.FMATERIALID
...
FROM 
  T_STK_INSTOCKENTRY TA1 
LEFT JOIN t_STK_InStock TA2 ON TA1.FID=TA2.FID
WHERE TA2.FDATE BETWEEN '{1}' AND '{2}' AND TA2.FDOCUMENTSTATUS ='C' AND TA1.FSTOCKID=281715
GROUP BY TA1.FMATERIALID) A15 ON A15.FMIDA15 = T.FMATERIALID 
...
FROM 
    T_PRD_INSTOCKENTRY AS TA5
LEFT JOIN T_PRD_INSTOCK  TA6 ON TA5.FID=TA6.FID 
WHERE TA6.FDATE BETWEEN '{1}' AND '{2}'  AND TA6.FDOCUMENTSTATUS ='C' AND TA5.FSTOCKID=281715
GROUP BY TA5.FMATERIALID) A19 ON A19.FMIDA19 = T.FMATERIALID
...
2 AS JINDU,
P.*,
row_number() over (ORDER BY P.FNUMBER) as FIDENTITYID into {0} 
...
WHERE T4.FDATE BETWEEN '{2}' AND '{3}'
...
LEFT JOIN T_BD_STOCK T3 ON T2.FSTOCKID= t3.fstockid
    ...
    GROUP BY T1.FBILLNO,T41.FNAME,T4.FNUMBER,T3.FNUMBER,T4.FOLDNUMBER,T32.FREFCOST,T5.FNAME,T1.FDATE,T4.FMATERIALID,T4.F_KD_TEXT
UNION ALL
    SELECT
				T1.FBILLNO AS BILLNO,
...
";
        ...
        string leftSQL = " group by  T3.FNUMBER,T3.FOLDNUMBER,T31.FNAME,T3.F_KD_TEXT,T4.FNAME,T32.FREFCOST,TA.FNAME";
...
sum(case when month(T1.FDATE)=2 and TS1.FISFREE='1' then  T2.FQTY else 0 end )  as 'FEBZENGPINNO',
sum(case when month(T1.FDATE)=1 and TS1.FISFREE='1' then  T2.FQTY else 0 end *  T9.FRefCost) AS 'FEBZENGPINAMT',
sum(case when month(T1.FDATE)=2 then  T2.FQTY else 0 end )  as 'FEBSUMTOTAL',
...
LEFT JOIN T_BD_OPERATORGROUPENTRY_L TC ON TB.FENTRYID=TC.FENTRYID
...
SELECT
	...
	T6.FBASEREMAINOUTQTY AS NOOUTNO,
...
WHERE T6.FBASEREMAINOUTQTY > 0  AND TA.FSTOCKID = '{1}'  AND T2.FMATERIALID= {2}
...
WHERE T3.FDOCUMENTSTATUS='C' and T1.FMATERIALID={1}
";
