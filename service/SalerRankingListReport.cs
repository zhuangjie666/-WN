...
namespace Kingdee.K3.WEINA.SCM.PlugIn.service
{
    ...
    public class SalerRankingListReport : SysReportBaseService
    {
        ...
        public override void Initialize()
        {
            ...
            //dcf.ByDecimalControlFieldName = "SEPSALENO";//显示的字段
            //dcf.ByDecimalControlFieldName = "SEPSALEAMT";//显示的字段
            //dcf.ByDecimalControlFieldName = "SEPZENGPINNO";//显示的字段
            //dcf.ByDecimalControlFieldName = "SEPZENGPINAMT";//显示的字段
            //dcf.ByDecimalControlFieldName = "SEPSUMTOTAL";//显示的字段
            //dcf.ByDecimalControlFieldName = "SEPSUMAMT";//显示的字段
            //dcf.ByDecimalControlFieldName = "SEPNOSENDNO";//显示的字段
            //dcf.ByDecimalControlFieldName = "SEPNOSENDAMT";//显示的字段
            //dcf.ByDecimalControlFieldName = "FPRICE";
            //dcf.DecimalControlFieldName = "JinDu";//用于精度控制的字段。
            //list.Add(dcf);
        ...
        public override void BuilderReportSqlAndTempTable(IRptParams filter, string tableName)
        {
            ...
            string sqlAll = sqlAllDetail.returnSQL4SalerRankListReport(searchCondition);
            ...
            {
                if (kv.Key.Equals(material))
                {
                    materialCondition = Convert.ToString(kv.Value);
                    searchCondition = " AND T3.FMATERIALID = " + materialCondition;
                    sqlAll = sqlAllDetail.returnSQL4SalerRankListReport(searchCondition);
                }
                ...
                if (kv.Key.Equals(materialGroupID))
                {
                    materialGroupCondition = Convert.ToString(kv.Value);
                    searchCondition = searchCondition + " AND T3.FMATERIALGROUP = " + materialGroupCondition;
                    sqlAll = sqlAllDetail.returnSQL4SalerRankListReport(searchCondition);
                }
            ...
            List<string> stockids = new List<string>();
             DynamicObjectCollection stockObject = customFilter["F_PAEZ_outStock"] as DynamicObjectCollection;
            foreach (DynamicObject stock in stockObject) {
                stockids.Add( Convert.ToString(stock["F_PAEZ_outStock_Id"]));
            }
            if(!string.IsNullOrEmpty(string.Join(",", stockids.ToArray()))){
                searchCondition = searchCondition + " AND T1.F_kd_Base IN (" + string.Join(",", stockids.ToArray())+")";
                sqlAll = sqlAllDetail.returnSQL4SalerRankListReport(searchCondition);
            }
           ...
           executeSQL = string.Format(sqlAll, tableName, selectedYear);
            ...
            //销售金额的更新
            string updateSaleNoAMTSQL = string.Format(sqlAllDetail.returnSQL4UpdateSaleNO(), tableName);
            DBUtils.Execute(this.Context, updateSaleNoAMTSQL);

            //赠品金额的更新
            string updateSaleZPAMTSQL = string.Format(sqlAllDetail.returnSQL4UpdateSaleZP(), tableName);
            DBUtils.Execute(this.Context, updateSaleZPAMTSQL);

            //合计金额更新
            string updateTotalAMTSQL = string.Format(sqlAllDetail.returnSQL4UpdateTotalAMT(), tableName);
            DBUtils.Execute(this.Context, updateTotalAMTSQL);

            //未发金额更新
            string updateNoSendAMTSQL = string.Format(sqlAllDetail.returnSQL4UpdateNoSendAMT(), tableName);
            DBUtils.Execute(this.Context, updateNoSendAMTSQL);
        }
        ...
        public override ReportTitles GetReportTitles(IRptParams filter)
        {
            ...
            titles.AddTitle("F_PAEZ_head_fmaterial", "");
            titles.AddTitle("F_PAEZ_head_fmaterialGroup", "");
            titles.AddTitle("F_PAEZ_head_saleGroup", "");
            titles.AddTitle("F_PAEZ_head_province", "");
            titles.AddTitle("F_PAEZ_head_outStock", "");
            if (customFilter["F_PAEZ_MATERIAL"] != null) {
                DynamicObject fmaterialObject = customFilter["F_PAEZ_MATERIAL"] as DynamicObject;
                titles.AddTitle("F_PAEZ_head_fmaterial", Convert.ToString(fmaterialObject["name"]));
            }
            if (customFilter["F_PAEZ_materialGroup"] != null) {
                DynamicObject fmaterialObject = customFilter["F_PAEZ_materialGroup"] as DynamicObject;
                titles.AddTitle("F_PAEZ_head_fmaterialGroup", Convert.ToString(fmaterialObject["name"]));
            }
            if (customFilter["F_PAEZ_saleGroup"] != null)
            {
                DynamicObject fmaterialObject = customFilter["F_PAEZ_saleGroup"] as DynamicObject;
                titles.AddTitle("F_PAEZ_head_saleGroup", Convert.ToString(fmaterialObject["name"]));
            }
            if (customFilter["F_PAEZ_agent4Province"] != null)
            {
                DynamicObject fmaterialObject = customFilter["F_PAEZ_agent4Province"] as DynamicObject;
                titles.AddTitle("F_PAEZ_head_province", Convert.ToString(fmaterialObject["name"]));
            }
            if (customFilter["F_PAEZ_agent4Province"] != null)
            {
                DynamicObject fmaterialObject = customFilter["F_PAEZ_agent4Province"] as DynamicObject;
            ...
            if (customFilter["F_PAEZ_outStock"] != null)
            {
                DynamicObjectCollection fmaterialObject = customFilter["F_PAEZ_outStock"] as DynamicObjectCollection;
                foreach(DynamicObject fmaterial in fmaterialObject){
                    DynamicObject stock = fmaterial["F_PAEZ_outStock"] as DynamicObject;
                    stockList.Add(Convert.ToString(stock["name"]));
                }
                titles.AddTitle("F_PAEZ_head_outStock", string.Join(",", stockList));
            }



            titles.AddTitle("F_PAEZ_head_year", selectedYear);
        ...
        private Dictionary<string, string> getOptionConditionFilter(IRptParams filter)
        {
            ...
            if(!int.Equals(0,Convert.ToInt32(customFilter["F_PAEZ_materialGroup_Id"]))){
                    materialGroup = Convert.ToString(customFilter["F_PAEZ_materialGroup_Id"]);
            }
            if(!int.Equals(0,Convert.ToInt32(customFilter["F_PAEZ_agent4Province_Id"]))){
                agent4Province = Convert.ToString(customFilter["F_PAEZ_agent4Province_Id"]);
            }
            ...
            if (!int.Equals(0, Convert.ToInt32(customFilter["F_PAEZ_material_Id"]))) {
                 material = Convert.ToString(customFilter["F_PAEZ_material_Id"]);//非必选
            }
            ...
            string stockids =string.Join(",", Convert.ToString(customFilter["F_PAEZ_outStock"])) ;//必选
            ...
            {
                conditionMap.Add("F_PAEZ_materialGroup", materialGroup);
            }

            if (!string.IsNullOrWhiteSpace(material))
            {
                conditionMap.Add("F_PAEZ_material", material);
            }
            ...
            //    conditionMap.Add("F_PAEZ_outStock", stockids);
            //}
        ...
        public override ReportHeader GetReportHeaders(IRptParams filter)
        {
            ...
            header.AddChild("FMATERIAL", new LocaleValue("产品编码", this.Context.UserLocale.LCID));
